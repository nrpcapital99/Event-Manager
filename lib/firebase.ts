...
const config = {apiKey:'AIzaSyDREl-3V5UP6OKI-ayDMTLnbARqwrB_2SU',authDomain:'teammanagement-882f0.firebaseapp.com',projectId:'teammanagement-882f0',storageBucket:'teammanagement-882f0.firebasestorage.app',messagingSenderId:'1048235426608',appId:'1:1048235426608:web:7ed996d4ec2212311fbef5',measurementId:'G-917DHBGMT2'};
...
// Loaded on demand: only a signed-in person calls the backend, and the SDK
// caches the Functions instance per app and region.
export async function mutate<T=unknown>(action:string,data:Record<string,unknown> = {}):Promise<T>{const { getFunctions, httpsCallable } = await import('firebase/functions');const result = await httpsCallable<Record<string,unknown>,T>(getFunctions(app,'asia-south1'),'workspace')({action,...data});return result.data;}
